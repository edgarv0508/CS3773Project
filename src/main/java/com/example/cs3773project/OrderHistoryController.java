...
public class OrderHistoryController implements Initializable {
    ...
    private void sortHistoryOrders(ObservableList<orderHistory> orders, String sortColumn, boolean isAsc) {
        if (SORT_BY_ORDER_DATE.equals(sortColumn)) {
            Collections.sort(orders, new Comparator<orderHistory>() {
                @Override
    ...
    public static void main(String[] args) {
        ObservableList<orderHistory> info = FXCollections.observableArrayList(
                new orderHistory("July 25, 2022", "Isaac Nguyen", 3.11, "Delivered" ), //changed
                new orderHistory( "May 26, 2022", "Isaac Neo", 1.11, "Delivered"),    //changed
                new orderHistory( "August 26, 2022", "Feirie Juno", 2.11, "Delivered") //changed
        );
        OrderHistoryController OrderHistoryController = new OrderHistoryController();
        System.out.println("=============SORT_BY_ORDER_DATE");
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_ORDER_DATE, false);
        System.out.println(info);
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_ORDER_DATE, true);
        System.out.println(info);
        System.out.println("=============SORT_BY_CUSTOMER");
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_CUSTOMER, false);
        System.out.println(info);
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_CUSTOMER, true);
        System.out.println(info);
        System.out.println("=============SORT_BY_AMOUNT");
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_AMOUNT, false);
        System.out.println(info);
        OrderHistoryController.sortHistoryOrders(info, SORT_BY_AMOUNT, true);
        System.out.println(info);
    }
}